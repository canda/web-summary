...
import { CopyIcon, DownloadIcon, LinkIcon, PlusIcon, ProfoundLogo } from "./icons";
    ...
    <S.SummaryBodyWrap>
      {blocks.map((block, index) => {
        ...
        if (index === 0 && lines.length === 1 && lines[0].length <= 90) {
          return <S.SummaryTitle key={`${block}-${index}`}>{lines[0]}</S.SummaryTitle>;
        }
        ...
        return <S.SummaryParagraph key={`${block}-${index}`}>{block}</S.SummaryParagraph>;
      })}
    </S.SummaryBodyWrap>
  );
}
      ...
      <S.AppShell>
        <S.Sidebar>
          <S.BrandRow>
            ...
            <S.GhostIcon type="button" onClick={handleNewSummary} aria-label="Start a new summary">
              <PlusIcon />
            </S.GhostIcon>
          </S.BrandRow>
        ...
        <S.ContentPanel>
          ...
          {!contentText && !isStreaming ? (
            <S.Hero>
              <S.HeroCopy>
                <S.Eyebrow>Website intelligence</S.Eyebrow>
              ...
              <S.HeroForm onSubmit={handleSubmit}>
                ...
                <S.PrimaryPill type="submit">Summarize</S.PrimaryPill>
              </S.HeroForm>
            ...
            <S.SummaryPanel>
              <S.SummaryScroll>
                <S.SummaryHeader>
                  <S.Eyebrow>{isStreaming ? "Streaming summary" : "Saved summary"}</S.Eyebrow>
                  <h2>{contentUrl}</h2>
                </S.SummaryHeader>
              ...
              <S.SummaryToolbar>
                ...
                {isStreaming ? (
                  ...
                  <>
                    <S.IconButton type="button" onClick={handleCopy} aria-label="Copy summary">
                      <CopyIcon />
                    </S.IconButton>
                    <S.IconButton type="button" onClick={handleDownload} aria-label="Download summary">
                      <DownloadIcon />
                    </S.IconButton>
                  </>
                )}
              ...
              {error ? <S.InlineErrorMessage>{error}</S.InlineErrorMessage> : null}
            </S.SummaryPanel>
          )}
        </S.ContentPanel>
      </S.AppShell>
